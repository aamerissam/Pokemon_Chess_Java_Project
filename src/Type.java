class Type {
    ...
    // Returns the species name of a Pokémon given its Pokédex number.
    public static String getEspece(int numPokedex) {
        if (numPokedex < 1 || numPokedex > espece.length) {
            return "Unknown";
        }
        return espece[numPokedex - 1]; // Pokédex numbers are 1-based, array is 0-based
    }

    // Returns the type name given its type index.
    public static String getNomType(int type) {
        if (type < 0 || type >= nomsType.length) {
            return "SANS";
        }
        return nomsType[type].toLowerCase(); // Return lowercase as specified
    }
    ...
    // Returns the type index given its name (case-insensitive).
